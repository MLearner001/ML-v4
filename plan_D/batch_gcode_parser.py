...
class NCParser:
  ...
  def parse_file(self, filepath: str, out_dir: Optional[str] = None) -> pd.DataFrame:
    ...
    for raw_line in lines:
      ...
      turn_match = re.search(r"\bTURN\s*=\s*([-\d]+)", line, re.IGNORECASE)
      turn_val = abs(int(turn_match.group(1))) if turn_match else 0
    ...
    # 1. Hapus Fitur Bising: Cmd_S
    if 'Cmd_S' in df.columns:
        df = df.drop(columns=['Cmd_S'])

    # 2. Ubah Posisi Absolut Menjadi Jarak Relatif
    for axis in ['X', 'Y', 'Z']:
        tgt_col = f'Tgt_{axis}'
